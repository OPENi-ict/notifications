...
var sendGCM = function (msg) {
   ...
   gcm.send(message, function (err, messageId) {
      if (err) {
         //this.logger.log("GCM: Something has gone wrong!");
      }
      else {
         //this.logger.log("GCM: Sent with message ID: ", messageId);
      }
   });
};
...
var deleteSub = function (msg, callback) {
   ...
   if(!openiUtils.isSubscriptionId(subId)){
      callback(null,
...
var createAppSubscriptions = function(msg){
   ...
   for( var r in result ){
      ...
      if(result[r]['@data'] !== undefined)
      {
         ...
         json.typeid = result[r]['@data']['openi_type'].trim()
